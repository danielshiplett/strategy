...
public class CreateTicketCommandHandler implements
    ...
    public CreateTicketResult handle(CreateTicketCommand command) {
        ...
        String taskQueue = temporalProperties.getWorkflowDefaults().getTaskQueue();
        WorkflowOptions workflowOptions = WorkflowOptions.newBuilder().setWorkflowId(name).setTaskQueue(taskQueue).build();
